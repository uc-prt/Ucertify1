<script>
    ...
    listItemCorrect = [],
    listItemAll = [];
    const icons = ["database-icon", "table-icon", "file-icon", "file-icon", "icomoon-new-24px-delete-1"];
            //let calcNodes 
            //let countInArray 
    let message = '';
    let jsRef;
    ...
    onMount(()=> {
        ...
        if (window.inNative && window.inNative != undefined) {
            var pre_timer4 = setTimeout(function () {
                window.postMessage(`height___${jQuery("#treemain0").height()}`, '*'); // increases the height of react native outer container
    ...
    beforeUpdate(()=> {
        if (window.inNative && window.inNative != undefined) {
            var pre_timer6 = setTimeout(function () {
        ...
        if (editorState) {
            var timer = setTimeout(function () {
                if (AI.find('#' + treeid, '.treeall')) {
                    ...
                    let result = ucTree.checkAns('#' + treeid, jQuery('#' + treeid).find('.treeall'));
                    onUserAnsChange(result);
                    ucTree.showAns(result.ans);
                    // shows the button of currect answer and your answer and check the answer and shows and does not allow the user to perform the task
                    ucTree.modeOn('on');
                }
                clearTimeout(timer);
    ...
    function parseXml(QXML, userXML) {
        ...
        setTimeout(function () {
            ...
            calcNodes(QXML.smxml.tree.__cdata);
        },100);
    }
    ...
    function setOptions(optionName, ID, level, isParant, pID) {
        ...
        // Defined but not called in this module so it may be remove
        // function loadInnerData(listItemCorrect, nextLevel, opened, selected) {
        //     for (var endblock = 1; endblock <= listItemCorrect['level'] - nextLevel; endblock++) {
        //         return (
        //             <li is tid={listItemCorrect.ID} correctans={listItemCorrect.correctAns} userans={listItemCorrect.userAns} data-jstree={'{"opened":' + opened + ',"selected":' + selected + ',"icon":"' + getIcon(listItemCorrect['level']) + '"}'}>
        //                 {listItemCorrect.item}
        //             </li>
        //         );
        //     }
        // }
    
        // used to create the Droppable area's default elements which will be seen just after render the component
        // function loadData() {
        //     let opened = "true",
        //         selected = false;
        //     // listItemCorrect cantains of tree which is not draggable
        //     let data = listItemCorrect.map(function (listItemCorrect) {
        //         if (listItemCorrect.item.length > 0) {
        //             if (listItemCorrect.isParant) {
        //                 selected = true;
        //             }
        //             // for native team
        //             if (window.inNative && window.inNative != undefined) {
        //                 return (
        //                     <li class='liItemIndex' is tid={listItemCorrect.ID} correctans={listItemCorrect.correctAns} userans={listItemCorrect.userAns} data-jstree={'{"opened":' + opened + ',"selected":' + selected + '"}'}>
        //                         {listItemCorrect.item}
        //                     </li>
        //                 );
        //             } else {
        //                 return (
        //                     // create the element of Droppable area and for answer matching value is checked from correctans attributes value with userans attributes value
        //                     <li class='liItemIndex' is tid={listItemCorrect.ID} correctans={listItemCorrect.correctAns} userans={listItemCorrect.userAns} data-jstree={'{"opened":' + opened + ',"selected":' + selected + ',"icon":"' + getIcon(listItemCorrect['level']) + '"}'}>
        //                         {listItemCorrect.item}
        //                     </li> 
        //                 );
        //             }
...
<svelte:head>
    <link rel="stylesheet" href="{themeUrl}pe-items/svelte/clsSMTree/libs/jstree.min.css"/>
    <script src="{themeUrl}pe-items/svelte/clsSMTree/libs/jstree.min.js"></script>
    <link rel="stylesheet" href="{themeUrl}pe-items/svelte/clsSMTree/libs/treeview.min.css" />
    <link rel="stylesheet" href={editor.baseUrlTheme + "pe-items/svelte/css/jstree/style.min.css"} />
</svelte:head>
<main>
    {#if state.blank}
        <div></div>
                ...
                <div id="treemain0" data-totalCorrectAns={state.totalCorrectAns} class="treemain">
                    ...
                    <div class="row-fluid tree">
                        ...
                        <div class="span6 treeall" data-showcheckbox={state.showCheckbox}>
                            <ul id="testme">
                                {#each listItemCorrect as listItemCorrect}
                                    {opened = "true"}
                                    {selected = false}
                                    {#if listItemCorrect.item.length > 0}
                                        {#if listItemCorrect.isParant}
                                        ...
                                        {/if}
                                        {#if (window.inNative && window.inNative != undefined)} 
                                                <li class='liItemIndex' tid={listItemCorrect.ID} correctans={listItemCorrect.correctAns} userans={listItemCorrect.userAns} data-jstree={'{"opened":' + opened + ',"selected":' + selected + '"}'}>
                                                    {listItemCorrect.item}
                                                </li>
                                        {:else}
                                                <li class='liItemIndex' tid={listItemCorrect.ID} correctans={listItemCorrect.correctAns} userans={listItemCorrect.userAns} data-jstree={'{"opened":' + opened + ',"selected":' + selected + ',"icon":"' + getIcon(listItemCorrect['level']) + '"}'}>
                                                    {listItemCorrect.item}
                                                </li> 
                            ...
                            <div class="span6 treecorrect" style= "width: 100%">
                                <ul>
                                    {#each state.listItemAll as data, i}
                                            <li key={i} class="liItemIndex"  tid={data.ID} seq={data.isParant.toString().replace(/,\s|{|}/gmi, '')} multi={data.multi} data-jstree={'{"icon":"' + getIcon(data.level) + '"}'}><span class="data_style">{data.item}</span><span class="delete_button"></span></li>
                                    {/each}
                                </ul>
                            </div>
                        </div>
