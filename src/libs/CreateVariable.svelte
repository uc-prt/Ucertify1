<script>    
	...
	import { Button, Dialog, Sidepanel } from 'svelte-mui/src';
    import { AH } from '../../helper/HelperAI.svelte';
    export let editorState;
    export let handleAlgoState;
   // export let labelview;
    export let l;
    ...
    export function analyzeValues(res) {
        ...
        state.analyzeValuesDialog = true;
        return;
    }
    ...
    export function showPane(afterCall) {
    ...
    function saveVariables(valid) {
        ...
        document.querySelectorAll('.new_variable').forEach(function(_this, i) {
            ...
            editorState.single_variables[i] = final_values;
        ...
        if (checkValidFunc(algo_str)) {
            state.algoQXML = algo;
            editorState.algo_qxml = algo;
            AH.selectAll("#algo_qxml", 'value', algo);
            console.log('-->',algo);
            editorState.variable_button = true;
    ...
    function saveAlgo() {
        ...
        dialogXml.split("\n").map(function(data, i) {
            editorState.single_variables[i] = data.trim();
        });
        if (checkRepeated()) return;
        if (unOrdered()) return;
        if (checkValidFunc(dialogXml)) {
            ...
            editorState.algo_qxml = AH.select("#algoxml_Dialog").value;      
    ...
    <Sidepanel right bind:visible={state.toggleHelpingDrawer}>
        <div tabIndex="0" role="button">
            <div class="relative" style="padding: 5px 10px; width: inherit; height: 100%;top:55px">
                ...
                <span 
                    on:click={closeHelpingPane} 
                    class="icomoon-arrow-right-6 font24 relative" 
                    style="top:5px;left: 62%;"
                ></span>
                <div class="outer_variables mt-xl clearboth">
                    <hr />
                    <div>
                    ...
                    <thead>
                        <tr>
                            {#each Object.keys(arr_val) as item, i}
                                <th key={i}>
                                    {item}
                                    <span style="font-size: 10px;">{arr_val[item]}</span>
                                </th>
                            {/each}
                        </tr>
                    </thead>
