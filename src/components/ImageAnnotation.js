 ...
 */
import JUI, {Draggable} from '../libs/javscript_helper/JUI';
        ...
        JS.listen('body', 'click', this.state.selector, (_this, event)=> {
            ...
            if (JS.find(selector, '.' + this.state.desc_pre_block, 'all').length == 0) {
                ...
                if (this.state.curdata.view == "0") {
                    ...
                    selector.innerHTML = (`<div class="pre-block ${this.state.desc_pre_block} ${this.state.curdata.color == "1" ? 'bl' : 'rl'}" style='overflow:scroll;height:auto;min-height:${JS.find(_this.parentElement, 'img').clientHeight + (JS.find(_this.parentElement, 'img').classList.contains('img-bordered') ? 8 : 0)}px;margin-bottom:${_this.parentElement.clientHeight - (JS.find(_this.parentElement, 'img').clientHeight + (JS.find(_this.parentElement, 'img').classList.contains('img-bordered') ? 8 : 0))}px'><div class="${this.state.list_container}"><div class="an_divp" d-m="${marker_index}">${marker_index == 0 ? `<div class="an_div" num="${number}">${number}</div>` : `<div class="an_div ${this.marker_icon[marker_index]}"></div>`}<div a_c='1'><b>Heading</b>: Sample Text</div></div></div></div>`);
                } else {
