...
define("tinymce/tableplugin/TableGrid", [
	...
	return function(editor, table, selectedCell) {
		...
		function removeCellSelection() {
			document.querySelectorAll('td[data-mce-selected],th[data-mce-selected]').forEach((elm)=> elm.removeAttribute('data-mce-selected'));
		}
		...
		function deleteTable() {
			...
			AI.select(AI.nextElm(table, "uc\\:caption"), 'remove');
			dom.remove(table);
		}
	...
	return function(editor) {
		...
		function deleteTable() {
			...
			function clearCell(cell) {
				document.querySelectorAll(cell).empty();
				Utils.paddCell(cell);
			}
		...
		self.table = function(isProps) {
			...
			function onSubmitTableForm() {
				...
				editor.undoManager.transact(function() {
					...
					if (data.uccenter) {
						if (!tableElm.parentElement.nodeName == "CENTER") {
							AI.wrap(tableElm, "<center></center>");
						}
					}
					else {
						if (tableElm.parentElement.nodeName == "CENTER") {
							var cnt = tableElm.parentElement.innerHTML;
							tableElm.parentElement.replaceWith(cnt);
						}
					}

					if(!data.uchover) {
						AI.addClass(tableElm, "remove_grey");
						//console.log("addclass");
					} else {
						AI.removeClass(tableElm, "remove_grey");
						//console.log("removeClass");
					}
					if(data.ucmiddle) {
						AI.find(tableElm, "td", {action: "addClass", actionData: "align-middle"});
						AI.find(tableElm, "thead tr", 'only').setAttribute("align","center");
					} else {
						AI.find(tableElm, "td", {action: "removeClass", actionData: "align-middle"});
						AI.find(tableElm, "thead tr", 'only').removeAttribute("align");
					}
					if(data.ucborder) {
						AI.addClass(tableElm, "table-bordered");
						//console.log("addclass");
					} else {
						AI.removeClass(tableElm, "table-bordered");
						//console.log("removeClass");
					}
					if (!AI.find(tableElm, "caption", 'all').length && !captionElm && data.caption) {
						captionElm = dom.create('caption');
						captionElm.innerHTML = !Env.ie ? '<br data-mce-bogus="1"/>' : '\u00a0';
						// captionElm = document.querySelector("caption");
						captionElm.setAttribute("width", data.captionWidth);
						captionElm.innerHTML = data.captionText;
						tableElm.prepend(captionElm);
						//$(captionElm).after("<br>");
					}
					else {
						let capElm = AI.find(tableElm, "caption");
						capElm ? capElm.setAttribute("width",data.captionWidth) : "";
						capElm ? capElm.innerHTML = data.captionText : "";
					}
			...
			if (isProps === true) {
				...
				if (tableElm) {
					data = {
						...
						caption: AI.find(tableElm, "caption", 'all').length == 1,
						'class': dom.getAttrib(tableElm, 'class'),
						'captionText': AI.find(tableElm, "caption") ? AI.find(tableElm, "caption").innerHTML : "",
						'captionWidth': AI.find(tableElm, "caption") ? AI.find(tableElm, "caption").getAttribute("width") : "",
						'uccenter': tableElm.parentElement.nodeName == "CENTER" ? true : false,
						'uchover': tableElm.classList.contains("remove_grey") ? false : true,
						'ucborder': tableElm.classList.contains("table-bordered") ? true : false,
						'ucmiddle': tableElm.querySelector("td").classList.contains("align-middle") ? true : false,
					};
					each('left center right'.split(' '), function(name) {
				...
				items: [
					{
						...
						items: (editor.settings.table_appearance_options !== false) ? [
							...
							{label: 'Caption width', name: 'captionWidth', type: 'textbox'},
							{label: 'Caption', name: 'caption', type: 'checkbox'},
							{label: 'Caption', name: 'captionText', type: 'textbox',maxWidth: 700},
							// {label: 'Center Align', name: 'uccenter', type: 'checkbox'}, @naveen Center Align checkbox
							{label: 'Hovering Effect', name: 'uchover', type: 'checkbox', checked: 'checked'},
							{label: 'Border', name: 'ucborder', type: 'checkbox', checked: ''},
							{label: 'Vertical Middle', name: 'ucmiddle', type: 'checkbox', checked: ''}
						] : [
							colsCtrl,
							rowsCtrl,
							{label: 'Width', name: 'width'},
...
define("tinymce/tableplugin/ResizeBars", [
	...
	return function(editor) {
		...
		function getBody() {
			var currBody = editor.getBody();
			return (currBody && isInline && currBody.ownerDocument) ? currBody.ownerDocument.body : currBody;
		}
		...
		function mouseDownHandler(e) {
			...
			//@naveen: because it was running twice
			if(document.body.classList.contains('ebook_item_text')) {
			...
			if (body && !body.contains(hoverTable) && hoverTable !== body) {
				return;
			}
...
define("tinymce/tableplugin/Plugin", [
	...
	function Plugin(editor) {
		...
		function insertTable(cols, rows) {
			...
			html = '<table id="__mce" class="uc-table"><tbody>';
			...
			editor.undoManager.transact(function() {
				...
				document.querySelectorAll('tr', tableElm).forEach(function(row, index) {
					editor.fire('newrow', {
						node: row
					});

					document.querySelectorAll('th,td', row).forEach(function(cell, index) {
						editor.fire('newcell', {
							node: cell
						});
					});
		...
		editor.on('PreInit', function() {
			...
			editor.serializer.addAttributeFilter('data-mce-cell-padding,data-mce-border,data-mce-border-color', function(nodes, name) {
					...
					while (i--) {
						nodes[i].setAttribute(name, null);
					}
				});
		});
