...
tinymce.PluginManager.add('lists', function(editor) {
	...
	editor.on('init', function() {
		...
		function indent(li) {
			var sibling, newList, listStyle,nested;
			...
			if((sibling.parentNode).getAttribute("nested")) {
				parentClass = (sibling.parentNode).getAttribute("nested").split(",");
				var remainingClass = (sibling.parentNode).getAttribute("nested").split(",");
				remainingClass.shift();
				nested = remainingClass.join();
			} else {
				parentClass = (sibling.parentNode.className) ? sibling.parentNode.className : "";
			}
			...
			sibling = li.previousSibling;
			if (sibling && sibling.nodeName == 'LI') {
				newList = dom.create(li.parentNode.nodeName); // nodeName remove after parentNode
				if(nested) {
					(newList).setAttribute("nested",nested);
				}
				newList.className = parentClass;
		...
		editor.on('keydown', function(e) {
			...
			/*if(e.keyCode == 13) {
				...
				var listElements = getSelectedListItems();
				var bookmark = createBookmark(selection.getRng(true));
				listElements[0].outerHTML = '<li><div><br data-mce-bogus="1"></div></li>';
				editor.nodeChanged();
				return;
			}*/
