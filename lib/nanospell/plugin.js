...
(function(exports, undefined) {
	...
	PluginManager.add('nanospell', function(editor, url) {
		...
		function showSuggestionsMenu(e, target, word) {
			console.trace('showSuggestionsMenu');
		...
		var caret_marker = String.fromCharCode(8) + String.fromCharCode(127) + String.fromCharCode(1) 
			...
			function validWordToken(word) {
				...
				if(editor.settings.nanospell_ignore_char) {
					var ignore_char =  editor.settings.nanospell_ignore_char.split(",");
					for(var i=0;i<ignore_char.length;i++){
						if (ignore_char[i] == word) return false;
					}
				}
				return true;
			}
		...
		function clearAllSpellCheckingSpans(base) {
			console.trace('clearAllSpellCheckingSpans');
			var i, node, nodes;
			var finished = false;
			while (!finished) {
				finished = true;
		...
		console.trace('document.body click handleBlur');
		...
		editor.on('keydown keypress', function(e) {
			console.trace('keydown keypress');
			editorHasFocus = true;
