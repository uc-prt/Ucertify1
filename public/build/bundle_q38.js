...
var app = (function () {
    ...
    CHART.checkAns = function(mid) {
        ...
        if (CHART.tempVar == 'u') {
            ...
            result = true;
            let elements = JS$1.select(mid).children;
        ...
        if (CHART.tempVar == 'u') {
            ...
            switch (_this) {
                ...
                case "dotplot": {
                    ...
                    let ansKey = JS$1.select(pElem).getAttribute('correctans').split('|').filter(function(element) {
                        return element != ''
                    }).sort().join('|');
                    // For short and removed blank values from user answer array.
                    let userKey = JS$1.select(pElem).getAttribute('userans').split('|').filter(function(element) {
                        return element != ''
                    }).sort().join('|');
                    if (ansKey != userKey) {
                    ...
                    userAnsXML += "<div id='" + JS$1.select(pElem).getAttribute('id') + "' userAns='" + JS$1.select(pElem).getAttribute('userans') + "'></div>\\n";
                    break;
                }
            }
        }
    ...
    CHART.showchildansdrag = function(mid, pElem, ansType, review) {
        let _this = JS$1.select(pElem).getAttribute('type'),
            elem = JS$1.select(pElem).getAttribute('id'),
            color = JS$1.select(pElem).getAttribute('color'),
            snapto = JS$1.select(pElem).getAttribute('snapto');
        let indx = [], indx1 = [], user = [];
        switch (_this) {
            case "column":
            case "line":
            ...
            case "dotplot": {
                if (ansType == 'c') {
                    ...
                    let ansKey = JS$1.select(pElem).getAttribute("correctans").split('|').sort();
                    ...
                    if (JS$1.select(pElem).getAttribute('userans') != '') {
                        // contains the user answer value after sorting in assending order
                        let uans = JS$1.select(pElem).getAttribute('userans').split('|').sort();
                        ...
                        let uans = JSON.parse(JS$1.select(pElem).getAttribute("[" + 'defaultans' + "]"));
                        for (let i = 0; i < uans.length; i++) {
    ...
    CHART.drawChartPreview = function(mid, review, uxml) {
        ...
        let chartData = JS$1.select(mid + ' [type]');
        let xaxis = [],
            userans, gpad = 0.2,
        ...
        let ada_data = JS$1.select("#answerID0").getAttribute('userans').split('|').filter(function(element) {
            return element != ''
        });
        ...
        if (!review && JS$1.select('#' + id).getAttribute('userans') == '') CHART.resetUserAns(id, type, CHART.defaultans); // reset the default value in user answer
        if (!simple_plot) {
            let cxWdth = JS$1.select('.highcharts-series-group rect').getAttribute('width');
    ...
    CHART.resetUserAns = function(id, type, defans) {
        ...
        JS$1.select('#' + id).setAttribute('userans', res);
    };
    ...
    CHART.updatePointPreview = function(type, id) {
        ...
        if (typeof chart1 == "object") {
            ...
            xaxis_data = JS$1.select('#' + id).getAttribute('xval'),
            // array 
            xaxis_arr,
            // contains the min value of 'y' after parsing it into integer
            ymin_preVal = parseInt(JS$1.select('#' + id).getAttribute('ymin')),
            // contains the yinterval value of 'y' after parsing it into integer
            yinterval_preVal = parseInt(JS$1.select('#' + id).getAttribute('yinterval')),
            ...
            userAnswer = JS$1.select('#' + id).getAttribute('userans');
            if (type == 'addPoint') {
                ...
                JS$1.select('#' + id).setAttribute('xval', '[' + xaxis_arr.join(',') + ']');
                ...
                // updates the user answer value
                JS$1.select('#' + id).setAttribute('userans', userAnswer);
            }
            if (type == 'removePoint') {
                let cans, ans, idx;
                if (xaxisVal > 1) {
                    ...
                    point_id && JS$1.select('[point-id = "' + point_id + '"]', 'remove');
                    ...
                    if (idx >= 0) {
                        ...
                        JS$1.select('#' + id).setAttribute('userans', '|' + cans + '|');
                    }
                    if (JS$1.select('#' + id).getAttribute('userans') == "|||") {
                        // when user answer value becomes '|||' then makes it blank
                        JS$1.select('#' + id).setAttribute('userans', '');
                    }
                }
            }
        }
    ...
    CHART.setUserAnswer = function(id, type, color, ansreview, snapto, indx1, user, correct) {
        ...
        let cid = JS$1.select('#' + id).parentElement.getAttribute('id');
        // width of chart container
        let wd = JS$1.find('#' + cid,'.highcharts-container').offsetWidth;
        ...
        if (!flagCorrect && ansreview) {
            ...
            JS$1.insert('#' + id, '<span class="correct_incorrect_icon" style="position: absolute;z-index:100;width:18px;height:18px;background:white;border-radius:12px;font-size: 21px;right: -7px;margin-top: -9px;"> <span class="' + wrong_ans + ' red" style="vertical-align:7px"></span></span>', 'afterbegin');
            ...
            if (ansreview) {
                ...
                JS$1.insert('#' + id, '<span class="correct_incorrect_icon" style="position: absolute;z-index:100;width:18px;height:18px;background:white;border-radius:12px;font-size: 21px;right: -7px;margin-top: -9px;"> <span class="' + corr_ans + '" style="vertical-align:7px;color:green;"></span></span>', 'afterbegin');
    ...
    CHART.setCorrectAnswer = function(id, type, color, snapto, indx, correct) {
        ...
        if ( JS$1.selectAll('.correct_incorrect_icon').length > 0) {
            // removes the correct incorrect icon container
            JS$1.select('.correct_incorrect_icon').remove();
    ...
    */
    ...
    /* clsSMChart/ChartPreview.svelte generated by Svelte v3.34.0 */

    const { console: console_1, document: document_1$1 } = globals;
    const file$1 = "clsSMChart/ChartPreview.svelte";

    function add_css$1() {
    	var style = element("style");
    	style.id = "svelte-1hjssff-style";
    	style.textContent = "#chart_modal.svelte-1hjssff .modal-body.svelte-1hjssff{max-height:350px}.native_chart_header.svelte-1hjssff.svelte-1hjssff{background:#e0e0e0;height:36px}.chart_preview_container.svelte-1hjssff.svelte-1hjssff{border:1px solid #ccc;overflow:visible !important}.native_add.svelte-1hjssff.svelte-1hjssff{background:#e0e0e0;width:auto;bottom:10px\n    }.native_add_text.svelte-1hjssff.svelte-1hjssff{bottom:2px;left:2px}.native_remove.svelte-1hjssff.svelte-1hjssff{padding:7.5px;background:#e0e0e0;width:auto;left:220px;top:7px}#chart_header_container.svelte-1hjssff.svelte-1hjssff{background:#e0e0e0;height:41px}.setdata.svelte-1hjssff.svelte-1hjssff{-webkit-appearance:none;cursor:pointer;margin-right:5px}\n/*# sourceMappingURL=data:application/json;charset=utf-8;base64, */";
    ...
    function create_else_block(ctx) {
    	...
    	let span3_data_original_title_value;
    	let t1;
    	let span5;
    	let span4;
    	let span5_title_value;
    	let span5_aria_label_value;
    	let span5_data_original_title_value;
    	let mounted;
    	let dispose;
    	...
    	const block = {
    		...
    		m: function mount(target, anchor) {
    			...
    			if (!mounted) {
    				dispose = [
    					listen_dev(span1, "click", /*click_handler_2*/ ctx[16], false, false, false),
    					listen_dev(span3, "click", /*click_handler_3*/ ctx[17], false, false, false)
    				];
    ...
    function create_if_block_1(ctx) {
    	...
    	const block = {
    		...
    		m: function mount(target, anchor) {
    			...
    			if (!mounted) {
    				dispose = [
    					listen_dev(span2, "click", /*click_handler*/ ctx[14], false, false, false),
    					listen_dev(span5, "click", /*click_handler_1*/ ctx[15], false, false, false)
    				];
    	...
    	dispatch_dev("SvelteRegisterBlock", {
    		...
    		type: "if",
    		source: "(538:12) {#if window.inNative}",
    		ctx
    	});
    ...
    // (579:24) {#if data.moduleType == 'column' ||  data.moduleType == 'line' ||  data.moduleType == 'histogram' ||  data.moduleType == 'dotplot'}
    function create_if_block$1(ctx) {
    	let div;
    	let label0;
    	let t0_value = "Point " + (/*i*/ ctx[36] + 1) + "";
    	let t0;
    	let t1;
    	let input0;
    	...
    	function change_handler(...args) {
    		return /*change_handler*/ ctx[20](/*i*/ ctx[36], ...args);
    	}
    	...
    	const block = {
    		c: function create() {
    			...
    			add_location(input0, file$1, 582, 36, 25861);
    			attr_dev(label0, "class", "col-6 text-body");
    			add_location(label0, file$1, 580, 32, 25736);
    		...
    		p: function update(new_ctx, dirty) {
    			...
    			if (dirty[0] & /*state*/ 4 && input1_value_value !== (input1_value_value = /*data*/ ctx[34].value)) {
    	...
    	dispatch_dev("SvelteRegisterBlock", {
    		...
    		type: "if",
    		source: "(579:24) {#if data.moduleType == 'column' ||  data.moduleType == 'line' ||  data.moduleType == 'histogram' ||  data.moduleType == 'dotplot'}",
    		ctx
    	});
    ...
    // (578:20) {#each  state.modalViewLayout as data, i}
    function create_each_block(ctx) {
    	...
    	const block = {
    		...
    		p: function update(ctx, dirty) {
    			if (/*data*/ ctx[34].moduleType == "column" || /*data*/ ctx[34].moduleType == "line" || /*data*/ ctx[34].moduleType == "histogram" || /*data*/ ctx[34].moduleType == "dotplot") {
    				if (if_block) {
    					if_block.p(ctx, dirty);
    				} else {
    	...
    	dispatch_dev("SvelteRegisterBlock", {
    		...
    		type: "each",
    		source: "(578:20) {#each  state.modalViewLayout as data, i}",
    		ctx
    	});
    ...
    function create_fragment$1(ctx) {
    	...
    	const block = {
    		c: function create() {
    			...
    			div2 = element("div");
    			button0 = element("button");
    			button0.textContent = `${l.add_point}`;
    			t6 = space();
    			button1 = element("button");
    			button1.textContent = `${l.del_row}`;
    			t8 = space();
    			div6 = element("div");
    			...
    			button2.textContent = `${l.cancel}`;
    			t11 = space();
    			button3 = element("button");
    			button3.textContent = `${l.ok_btn}`;
    			...
    			attr_dev(div0, "class", div0_class_value = "" + (null_to_empty("drag-resize chart_preview_container " + /*state*/ ctx[2].moduleType) + " svelte-1hjssff"));
    		...
    		m: function mount(target, anchor) {
    			...
    			if (!mounted) {
    				dispose = [
    					...
    					listen_dev(button3, "click", /*modifyUxmlOnKey*/ ctx[4], false, false, false)
    				];
    		...
    		p: function update(ctx, dirty) {
    			...
    			if (!current || dirty[0] & /*state*/ 4 && div0_class_value !== (div0_class_value = "" + (null_to_empty("drag-resize chart_preview_container " + /*state*/ ctx[2].moduleType) + " svelte-1hjssff"))) {
    			...
    			if (dirty[0] & /*state, updateInputElm*/ 516) {
    ...
    class ChartPreview extends SvelteComponentDev {
    	constructor(options) {
    		super(options);
    		if (!document_1$1.getElementById("svelte-1hjssff-style")) add_css$1();
...
//# sourceMappingURL=bundle_q38.js.map
