...
var app = (function () {
    ...
    ucMlid.removeUserAns = function() {
        AH$1.find("#matchmain","#lines, .matchlist-delete",{ action:'remove'});
        AH$1.selectAll("#matchmain div[data-userans]").forEach((_this)=> {
    ...
    ucMlid.update_XMLValue = function() {
        ucMlid.xmlDom = AH$1.select("#special_module_xml").value;
        ucMlid.multimatch = AH$1.select("#matchType").value;
        if(AH$1.select("#multimatch").checked) {
            ucMlid.multimatch = 1;
        }
        
        let ucMlidXMLDom = AH$1.findChild(ucMlid.xmlDom, 'matchlist');
        AH$1.setAttr(ucMlidXMLDom, {
            "listheading1": AH$1.select("#list1").value,
            "listheading2": AH$1.select("#list2").value,
            "multimatch": ucMlid.multimatch
        });
        ucMlidXMLDom.innerHTML = "<![CDATA[\n" + AH$1.select("#matchList").value + "\n]]>";
        AH$1.select("#special_module_xml", 'value', formatXml(ucMlid.xmlDom.xml ? ucMlid.xmlDom.xml : (new XMLSerializer()).serializeToString(ucMlid.xmlDom[0])) );
    };



    ucMlid.remove_lines = function(mlid) {
        AH$1.select(mlid+' #remMatchLine','remove');
        AH$1.listen('body', 'click','.matchlist-delete', (element)=> {
            let rem = {
            ...
            AH$1.select(`[id="${rem.base[0]}"]`, 'attr', {"data-userans" : rem.userans});
            ...
            AH$1.remove(`[base='${element.getAttribute('base')}']`);
    ...
    AH$1.listen('body','click','.donotshowdialo',(_this) => {
    ...
    ucMlid.showUserAns = function(mlid) {
        let top1 = 0;
        match_lines = [];
        if(AH$1.select(mlid).nodeName != undefined)
        ucMlid.multimatch = AH$1.select(mlid).getAttribute("multimatch"); // Replaced
        ...
        function drop1(event,ui) {
            ...
            let list;
            let clr = "black";
            if(AH$1.select('#main-page').length > 0 && AH$1.select('#main-page').getAttribute('mode') == "bla" ) clr = "white";
            ...
            str += '</svg>';    
            AH$1.insert(mlid, circle, 'afterbegin');
            AH$1.insert(mlid, str, 'afterbegin');
    ...
    ucMlid.bindKeyup = function(mlid) {
        ...
        AH$1.find(".row-fluid", ".selmatch", {action: 'removeClass', actionData: 'selmatch'});
        ...
        if(AH$1.select(mlid).nodeName != undefined)
        is_multimatch = AH$1.select(mlid).getAttribute("multimatch");
        ...
        function loseFocus() {
            AH$1.find(mlid, ".copiedclr", 'all').forEach((_elm)=> {
        ...
        function activateClass(cls) {
            AH$1.find(mlid, "." + cls, 'all').forEach((_elm)=> {
        ...
        function navigateDown() {
            const a = AH$1.selectAll("#matchmain .ks").length;
            //count_prev = ((count == 0) ? (a - 1) : (count - 1)); @eslint issues solved
            const ks_fill = AH$1.find(mlidVar,".ks:nth(" + count + ")");
        ...
        function copyDraggable() {
            let copy_drag = AH$1.selectAll(mlid+" .ks:focus");
            AH$1.find(mlid,".list1",'all').forEach((_this)=> {
                _this.classList.remove("copiedclr");
            });
            AH$1.find(mlid,".list4",'all').forEach((_this)=> {
                _this.classList.remove("copiedclr");
            });
            copied_id = copy_drag[0].id;
            //copy_drag.className = "copiedclr";
            AH$1.select(copy_drag[0],'addClass','copiedclr');
        }

        ucMlid.cleanTitle = function(){
            AH$1.find(ucMlid.ajax_eId, '.list1', 'all').forEach((_this) => {
        ...
        ucMlid.pasteDraggable = function() {
            ...
            /*    _ui_drag.after(_ui_drag.clone().addClass("clone").css({ "position": "absolute", "top": _ui_drag.position().top, "left": _ui_drag.position().left, "width": _ui_drag.width() + 20, "height": _ui_drag.height() + 15 })); // Need to fixed it */
                ...
                let _ui_drag_cloned = AH$1.clone(_ui_drag);
                _ui_drag_cloned.classList.add("clone");

                AH$1.setCss(_ui_drag_cloned,{
                    "position": "absolute",
                ...
                if (AH$1.selectAll('#main-page').length > 0 && AH$1.select('#main-page').getAttribute("mode") == "bla") clr = "white";
                ...
                if (userans.length > 0){
                    ...
                    tempUdata.map((id,index)=> {
                        attachedText = ucMlid.getTitle(AH$1.select("#"+id), attachedText, index);
                    });
        ...
        ucMlid.pasteDraggableList3 = function() {  
            ...
            var _dropthis = AH$1.find(mlid,".ks:focus");
            if (copied_id != "") {
                //var _ui_drag = AH.find(mlid,"#" + '[id="'+copied_id+'"]' + "");
                var _ui_drag = AH$1.select(mlid+' #'+copied_id);
                _ui_drag.classList.remove('copiedclr');
                _dropthis.style.position="relative";
        ...
        function removeDraggable() {
            var _removethis = AH$1.find(mlid,".ks:focus");
            if(_removethis.classList.contains('ui-draggable') && _removethis.getAttribute("data-userans") != "") {    
                ...
                var element = "";
                AH$1.find(mlid,".matchlist-delete","all").forEach((_this)=>{
                ...
                var userans = AH$1.find(mlid,'[id="'+rem.base[0]+'"]').getAttribute('data-userans').split(',');
                ...
                (parseInt(l2[l2.length - 2]) + 10) + "," + AH$1.select(element).getAttribute('base');
        ...
        function checkFocus(list) {
            var is_focus = false;
            let elements = AH$1.find(mlid, "." + list, 'all');
            let focus_ele = AH$1.select(":focus");
        ...
        function tabMove(element, type = 1) {
            if (element.id) {
                let ks_element = AH$1.selectAll('.ks');
                let focus_index = -1;
        ...
        if (typeof hotkeys$1 == "function") {
            ...
            hotkeys$1("down,up,left,right,alt+down,enter,delete,esc", 'matchlist', function(event, handler) {
                switch (handler.key) {
                    ...
                    case 'right':
                        if (checkFocus("ks")) {
                            let focus_ele = AH$1.select(".ks:focus");
                            let ks_last = AH$1.selectAll(".ks")[AH$1.selectAll(".ks").length-1];
                            if(focus_ele.id == ks_last.id) {
                                AH$1.selectAll(".ks")[0].focus();
                            } else {
                                tabMove(focus_ele,1);
                            }
                            event.preventDefault();
                    ...
                    case 'left':
                        if (checkFocus("ks")) {
                            let focus_ele = AH$1.select('.ks:focus');
                            let ks_element = AH$1.selectAll('.ks')[0];
                            if (focus_ele.id == ks_element.id) {
                                AH$1.selectAll(".ks")[AH$1.selectAll(".ks").length].focus();
                            } else {
                    ...
                    case 'esc':
                        if (checkFocus("ks")) {
                            ...
                            focusLast();
                            //jQuery('.ks').last().blur();
                            AH$1.selectAll(".ks")[AH$1.selectAll(".ks").length -1].blur();
                        }
                        break;
                }
            });
    ...
    ucMlid.showAns = function(mlid) {
        if(AH$1.select(mlid).nodeName != undefined)
        ucMlid.multimatch = AH$1.select(mlid).getAttribute("multimatch");
        //var answer_ele = ".list1"; @eslint issues solved
        if (ucMlid.multimatch < 2) {
            var str = '<svg id="lines">';
            ucMlid.f = 0;
            AH$1.selectAll(mlid +' .list1').forEach((_this)=> {
                ...
                if (_this.getAttribute("data-userans") != '') {
                    ...
                    for (let i = 0; i < userAns.length; i++) {
                        if (userAns[i] != '') {
                            const top2 = parseInt(AH$1.find(mlid ,'#' + userAns[i]).offsetTop + AH$1.find(mlid,'#' + userAns[i]).clientHeight / 2);
                            const left = parseInt(AH$1.find(mlid,'#' + userAns[i]).offsetLeft);
                            const userans = top2 + "_" + left;
                            const circle = '<div class="matchlist-delete center-block" d="M' + (parseInt(right) + 20) + ',' + top1 + 'C' + (parseInt(right) + 88) + ',' + top1 + ',' + (parseInt(right) + 88) + ',' + top2 + ',' + (parseInt(left) - 10) + ',' + top2 + '" base="' + _this.getAttribute('id') + '_' + AH$1.find(mlid,'#' + userAns[i]).getAttribute('id') + '" style="left:' + (parseInt(right) ) + 'px;top:' + (top1  - 9)+ 'px">&times</div>';
                            ...
                            const list = index + "," + userans + "," + _this.getAttribute('id') + "_" + AH$1.select(mlid +' #' + userAns[i]).getAttribute('id');
            ...
            if (ucMlid.f == 1) AH$1.insert(mlid,str,'afterbegin');
        } else {
    ...
    ucMlid.checkAns = function(mlid) {
        ...
        if (mlid == "#matchmain") {
            AH$1.selectAll(mlid+" "+answer_ele).forEach((_this)=> {
                ...
                if (typeof calculatePoint != "undefined") {
                    calculatePoint(AH$1.select("#matchmain").getAttribute("totalcorrectans"), temp);
                }
            ...
            AH$1.find(mlid, answer_placeholder, 'all').forEach((_this)=> {
                var ans = '';
                //jQuery(mlid).find(answer_ele + '[data-userans*="' + _this.attr('id') + '"]').each(function() {
                AH$1.find(mlid, answer_ele + `[data-userans*="${_this.getAttribute('id')}"]`, 'all').forEach((_this2)=> {
            ...
            if (result) {
                ...
                if (typeof(is_sm) != "undefined") AH$1.showmsg("Correct");
                return(rest);
            } else {
                //AH.select("#answer").checked = false; ## Fixed in HelperAI.svelte
                if (typeof(is_sm) != "undefined") AH$1.showmsg("Incorrect");
                return(rest);
            }
    ...
    ucMlid.showMultimatchAnswer = function(mlid, is_not_review) {
       // var top1 = 0; @eslint issues solved
        ...
        AH$1.find(mlid, ' .list3', 'all').forEach((_this)=> {
            AH$1.select(_this,'removeClass','dropped');
            _this.innerHTML = "Place here";
            AH$1.select(_this,'css',{backgroundColor:'#fff'});
        });
        ...
        AH$1.find(mlid, '.list3', 'all').forEach((_this)=> {
            ...
            if (_this.getAttribute("data-userans").length > 0) {
                ...
                for (let i = 0; i < userAns.length; i++) {
                    if (userAns[i] != '') {
                        const str = AH$1.find(mlid,'#' + userAns[i]).innerHTML;
                        _this.innerHTML = str;
    ...
    ucMlid.showAllCorrectAns = function(mlid) {
        AH$1.selectAll(mlid+' .list3').forEach((_this) => {
            const correctAns = _this.getAttribute("data-correctans").split(',');
            for (let i = 0; i < correctAns.length; i++) {
                if (correctAns[i] != '') {
                    const str = AH$1.find(mlid,'#' + correctAns[i]).innerHTML;
                    _this.innerHTML = str;
                    AH$1.select(_this,'addClass','dropped');
                }
            }
        });
    };
    ...
    ucMlid.showAllAns = function(mlid) {
        AH$1.remove("#lines");
        AH$1.remove('.correct_incorrect_icon');
        ...
        if (ucMlid.multimatch < 2) {
            ...
            AH$1.find(mlid ,".list1","all").forEach((_this)=>{
                ...
                for (let i = 0; i < correctAns.length; i++) {
                    if (correctAns[i] != '') {
                        const top2 = AH$1.find(mlid,'#' + correctAns[i]).offsetTop + AH$1.find(mlid,'#' + correctAns[i]).clientHeight / 2 - 5;
                        const left = AH$1.find(mlid,'#' + correctAns[i]).offsetLeft;
                ...
                if(_this.getAttribute("data-userans").length > 0) {
                    ...
                    for (let i = 0; i < userAns.length; i++) {
                        if (userAns[i] != '') {
                            const top2 = AH$1.find(mlid,'#' + userAns[i]).offsetTop + AH$1.find(mlid,'#' + userAns[i]).clientHeight / 2 + 5;
                            const left = AH$1.find(mlid,'#' + userAns[i]).offsetLeft;
                            ...
                            setTimeout(function() {
                                matchedTitle = ucMlid.getTitle(AH$1.select('#' + userAns[i]), matchedTitle, i, true, _this.textContent);
                            },200);
        ...
        AH$1.selectAll(mlid +" "+ answer_ele).forEach((_this)=>{
            ...
            if (ucMlid.multimatch == 2) {
                ...
                AH$1.selectAll(mlid + ' .list1[data-correctans*="' + _this.getAttribute('id') + '"]').forEach((_this)=>{
            ...
            AH$1.insert(_this,'<span class="correct_incorrect_icon" style="position:absolute;width:19px;height:19px;right:-9px;top:-9px;background:white;border-radius:15px 12px 12px;font-size:21px;"> ' + droped_value_indicator_html + '</span>','beforeend');
        });
    };
        ...
        AH$1.selectAll(mlid +' .list1').forEach((_this)=> {
            ...
            for (let i = 0; i < correctAns.length; i++) {
                if (correctAns[i] != '') {
                    ...
                    const left = AH$1.find(mlid,'#' + correctAns[i]).offsetLeft;
                    str += '<path fill="none" d="M' + (parseInt(right) ) + ',' + (parseInt(top1) - 5) + 'C' + (parseInt(right) + 83) + ',' + (parseInt(top1) - 5) + ',' + (parseInt(right) + 83) + ',' + top2 + ',' + (parseInt(left) - 10) + ',' + top2 + '" marker-end="url(#trngle)" class="correct"></path>';
                    matchedTitle = ucMlid.getTitle(AH$1.select('#' + correctAns[i]), matchedTitle, i, _this.textContent);
                }
            }
    ...
    ucMlid.showYour = function(mlid) {
        ...
        AH$1.find(mlid,answer_ele,"all").forEach((_this)=> {
            //const self = jQuery(this);
            const self_id = _this.getAttribute('id');

            let isCorrect = 1;
            AH$1.find(mlid,'.list1[data-correctans*="' + _this.getAttribute('id') + '"]','all').forEach((_this)=> {
                isCorrect = ucMlid.checkAnswerStatus(isCorrect, _this, self_id);
            });
            ...
            AH$1.insert(_this,'<span class="correct_incorrect_icon" style="position:absolute;width:19px;height:19px;right:-9px;top:-9px;background:white;border-radius:15px 12px 12px;font-size:21px;"> ' + droped_value_indicator_html + '</span>',"afterbegin");
        });
    }; 
    ...
    ucMlid.modeOn = function(modeType) {
        ...
        AH$1.selectAll('.test','addClass','h');
        //jQuery(ucMlid.ajax_eId).find('.review_2, .review_default').addClass('h');
       
        AH$1.find(ucMlid.ajax_eId,'.review_2, .review_default',{
            action:'addClass',
            actionData:'h'
        });
        if (modeType) {
            //jQuery('.review').removeClass('h');
            AH$1.selectAll('.review','removeClass','h');
            setTimeout(function() {
                if (typeof hotkeys$1 == "function") {
                    hotkeys$1.unbind('down,up,left,right,alt+down,enter,delete,esc', 'matchlist');
                }
                //jQuery(ucMlid.ajax_eId).find(".selmatch").removeClass("selmatch");
                AH$1.selectAll(ucMlid.ajax_eId+"  .selmatch",'removeClass','selmatch');
                ucMlid.unBindLab();
                ucMlid.showAllAns(ucMlid.ajax_eId);
                ...
                // });
                AH$1.selectAll(ucMlid.ajax_eId+' #sm_controller button').forEach((_this) => {
                ...
                AH$1.find(ucMlid.ajax_eId,'#sm_controller_default button','all').forEach((_this)=> {
            ...
            AH$1.selectAll('.review','addClass','h');
            //jQuery('.test').removeClass('h');
            AH$1.selectAll('.test','removeClass','h');
            setTimeout(function() {
                ucMlid.bindLab();
    ...
    ucMlid.unBindLab = function() {
        ...
        AH$1.find(ucMlid.ajax_eId,'#lines, .matchlist-delete',{
            action: 'remove'
        });
    ...
    ucMlid.modifyClass = function(self, type) { //  Not catched when call
        ...
        AH$1.find(ucMlid.ajax_eId,"#sm_controller button, #sm_controller_default button","all"); // Will Replaced
        if(type){
    ...
    ucMlid.bindLab = function() {
        ...
        //     action:'remove'
        // })
        AH$1.remove(ucMlid.ajax_eId+' #lines,.correct,.lines,.correct_incorrect_icon');
    ...
    ucMlid.editor_validtion = function(editing_type) {
        if (editing_type == 1 || editing_type == 2 || editing_type == 3) {
            ...
            if ($list1 == "" || $list2 == "" || $matchList == "") {
                AH$1.showmsg("Please Fill The Heading of Lists and Question", 1);
                return false;
            }
            return true;
        }
    ...
    function add_css$4() {
    	...
    	style.textContent = ".u-sr-only.svelte-bi3u6x{position:absolute;left:-10000px;top:auto;width:1px;height:1px;overflow:hidden}@media(max-width:500px){.shuffle.svelte-bi3u6x{text-align:center}}\n/*# sourceMappingURL=data:application/json;charset=utf-8;base64, */";
    ...
    function create_if_block_1(ctx) {
    	...
    	const block = {
    		c: function create() {
    			...
    			add_location(div, file$4, 778, 4, 22591);
    		},
    		m: function mount(target, anchor) {
    			insert_dev(target, div, anchor);
    ...
    function create_else_block(ctx) {
    	...
    	const block = {
    		c: function create() {
    			...
    			add_location(div0, file$4, 901, 6, 26718);
    			attr_dev(div1, "class", "row-fluid shuffleList1");
    			add_location(div1, file$4, 863, 5, 25613);
    		},
    		m: function mount(target, anchor) {
    		...
    		p: function update(ctx, dirty) {
    			...
    			if (dirty[0] & /*list2, isReview, setList2Html, alphabet*/ 266497) {
    ...
    // (822:4) {#if (multimatch == 0 || multimatch == 1)}
    function create_if_block$3(ctx) {
    	let div3;
    	let div0;
    	let div0_dragable_value;
    	let t0;
    	let div1;
    	let t1;
    	let div2;
    	...
    	const block = {
    		c: function create() {
    			...
    			attr_dev(div0, "dragable", div0_dragable_value = /*isReview*/ ctx[0] === true ? 0 : 1);
    			add_location(div0, file$4, 823, 5, 24499);
    			attr_dev(div1, "class", "span3");
    			add_location(div1, file$4, 842, 5, 25093);
    			attr_dev(div2, "class", "span4 shuffleList2");
    			add_location(div2, file$4, 843, 5, 25124);
    			attr_dev(div3, "class", "row-fluid");
    			add_location(div3, file$4, 822, 4, 24470);
    		},
    		m: function mount(target, anchor) {
    			insert_dev(target, div3, anchor);
    		...
    		p: function update(ctx, dirty) {
    			if (dirty[0] & /*list1, isReview, setList1Html*/ 131201) {
    			...
    			if (dirty[0] & /*isReview*/ 1 && div0_dragable_value !== (div0_dragable_value = /*isReview*/ ctx[0] === true ? 0 : 1)) {
    				attr_dev(div0, "dragable", div0_dragable_value);
    			}
    	...
    	dispatch_dev("SvelteRegisterBlock", {
    		...
    		type: "if",
    		source: "(822:4) {#if (multimatch == 0 || multimatch == 1)}",
    		ctx
    	});
    ...
    function create_each_block_3(ctx) {
    	...
    	const block = {
    		c: function create() {
    			...
    			add_location(div0, file$4, 867, 9, 25757);
    			attr_dev(span0, "class", "span4");
    			add_location(span0, file$4, 866, 8, 25727);
    			attr_dev(div1, "id", div1_id_value = /*data*/ ctx[54].id);
    			attr_dev(div1, "class", "arrow");
    			add_location(div1, file$4, 879, 9, 26127);
    			attr_dev(span1, "class", "span3");
    			add_location(span1, file$4, 878, 8, 26097);
    			...
    			add_location(div2, file$4, 882, 9, 26220);
    			attr_dev(span2, "class", "span4");
    			add_location(span2, file$4, 881, 8, 26190);
    			attr_dev(div3, "key", div3_key_value = /*i*/ ctx[56]);
    			attr_dev(div3, "class", "row-fluid");
    			add_location(div3, file$4, 865, 7, 25687);
    		},
    		m: function mount(target, anchor) {
    			insert_dev(target, div3, anchor);
    	...
    	dispatch_dev("SvelteRegisterBlock", {
    		...
    		type: "each",
    		source: "(865:6) {#each list1 as data,i}",
    		ctx
    	});
    ...
    function create_each_block_2(ctx) {
    	...
    	const block = {
    		c: function create() {
    			...
    			attr_dev(div, "dragable", div_dragable_value = /*isReview*/ ctx[0] === true ? 0 : 1);
    			attr_dev(div, "draggable", div_draggable_value = /*isReview*/ ctx[0] === true ? "false" : "true");
    			...
    			add_location(div, file$4, 903, 8, 26808);
    		},
    		m: function mount(target, anchor) {
    			insert_dev(target, div, anchor);
    		...
    		p: function update(ctx, dirty) {
    			...
    			if (dirty[0] & /*isReview*/ 1 && div_dragable_value !== (div_dragable_value = /*isReview*/ ctx[0] === true ? 0 : 1)) {
    				attr_dev(div, "dragable", div_dragable_value);
    			}

    			if (dirty[0] & /*isReview*/ 1 && div_draggable_value !== (div_draggable_value = /*isReview*/ ctx[0] === true ? "false" : "true")) {
    	...
    	dispatch_dev("SvelteRegisterBlock", {
    		...
    		type: "each",
    		source: "(903:7) {#each list2 as data,i}",
    		ctx
    	});
    ...
    function create_each_block_1(ctx) {
    	...
    	const block = {
    		c: function create() {
    			...
    			add_location(div, file$4, 825, 8, 24609);
    		},
    		m: function mount(target, anchor) {
    			insert_dev(target, div, anchor);
    			html_tag.m(raw_value, div);
    	...
    	dispatch_dev("SvelteRegisterBlock", {
    		...
    		type: "each",
    		source: "(825:6) {#each list1 as data,i}",
    		ctx
    	});
    ...
    function create_each_block(ctx) {
    	...
    	const block = {
    		c: function create() {
    			...
    			add_location(div, file$4, 845, 6, 25192);
    		},
    		m: function mount(target, anchor) {
    			insert_dev(target, div, anchor);
    			html_tag.m(raw_value, div);
    	...
    	dispatch_dev("SvelteRegisterBlock", {
    		...
    		type: "each",
    		source: "(845:5) {#each list2 as data,i}",
    		ctx
    	});
    ...
    // (935:4) <Button style={'position:relative;left:21px;bottom:6px;'} on:click={()=>{state.dropDialog = false}}>
    function create_default_slot_1(ctx) {
    	...
    	const block = {
    		c: function create() {
    			...
    			add_location(span, file$4, 935, 54, 27795);
    			attr_dev(i_1, "class", "mi mi-close");
    			set_style(i_1, "font-size", "1.5rem");
    			add_location(i_1, file$4, 935, 5, 27746);
    		},
    		m: function mount(target, anchor) {
    	...
    	dispatch_dev("SvelteRegisterBlock", {
    		...
    		type: "slot",
    		source: "(935:4) <Button style={'position:relative;left:21px;bottom:6px;'} on:click={()=>{state.dropDialog = false}}>",
    		ctx
    	});
    ...
    // (926:1) <Dialog    bind:visible={state.dropDialog}    width="350px"   height="271px"   style="background: #fff; border-radius: 5px;"  >
    function create_default_slot(ctx) {
    	...
    	const block = {
    		c: function create() {
    			...
    			add_location(div0, file$4, 932, 3, 27521);
    			attr_dev(div1, "class", "float-end float-right");
    			add_location(div1, file$4, 933, 3, 27600);
    			...
    			add_location(img, file$4, 941, 4, 27899);
    			add_location(br, file$4, 945, 4, 28020);
    			attr_dev(input, "type", "checkbox");
    			set_style(input, "top", "2px");
    			attr_dev(input, "class", "relative donotshowdialog");
    			attr_dev(input, "id", "dropId");
    			add_location(input, file$4, 947, 5, 28055);
    			attr_dev(label, "for", "dropId");
    			add_location(label, file$4, 948, 5, 28148);
    			attr_dev(span, "class", "mt-2");
    			add_location(span, file$4, 946, 4, 28030);
    			add_location(div2, file$4, 940, 3, 27889);
    			add_location(div3, file$4, 939, 2, 27880);
    		},
    		m: function mount(target, anchor) {
    	...
    	dispatch_dev("SvelteRegisterBlock", {
    		...
    		type: "slot",
    		source: "(926:1) <Dialog    bind:visible={state.dropDialog}    width=\\\"350px\\\"   height=\\\"271px\\\"   style=\\\"background: #fff; border-radius: 5px;\\\"  >",
    		ctx
    	});
    ...
    function create_fragment$4(ctx) {
    	...
    	const block = {
    		c: function create() {
    			...
    			add_location(button2, file$4, 808, 6, 23915);
    			attr_dev(div1, "class", "btn-group clearfix review_default h");
    			attr_dev(div1, "id", "sm_controller_default");
    			add_location(div1, file$4, 805, 5, 23346);
    			attr_dev(div2, "class", div2_class_value = /*btnflag*/ ctx[13] == 0 ? "h" : "");
    			add_location(div2, file$4, 804, 4, 23305);
    			attr_dev(div3, "class", "heading");
    			add_location(div3, file$4, 813, 6, 24233);
    			attr_dev(div4, "class", "span4");
    			add_location(div4, file$4, 812, 5, 24207);
    			attr_dev(div5, "class", "span3");
    			add_location(div5, file$4, 815, 5, 24292);
    			attr_dev(div6, "class", "heading");
    			add_location(div6, file$4, 817, 6, 24349);
    			attr_dev(div7, "class", "span4");
    			add_location(div7, file$4, 816, 5, 24323);
    			attr_dev(div8, "class", "row-fluid");
    			add_location(div8, file$4, 811, 4, 24178);
    			...
    			add_location(div9, file$4, 787, 3, 22798);
    			add_location(center, file$4, 776, 2, 22557);
    			attr_dev(div10, "id", "previewSection");
    			attr_dev(div10, "class", "px-2 svelte_items_testing");
    			add_location(div10, file$4, 775, 4, 22495);
    			add_location(main, file$4, 774, 0, 22484);
    		},
    		l: function claim(nodes) {
    			throw new Error("options.hydrate only works if the component was compiled with the `hydratable: true` option");
    		},
    				...
    				onDragEnd: event => {
    					...
    					if (!ucMlid.is_valid_drop) {
    						if (ucMlid.sinfo) {
    							...
    							// if (!UCINFO.isIphone) {
    							if (typeof AH.alert == "function") AH.showmsg("While dropping a component, keep your mouse pointer on the drop area. Drop area must be compatible with the component you are dropping.", 3000);
