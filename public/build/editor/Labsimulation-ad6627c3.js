...
import { S as SvelteComponentDev, i as init, s as safe_not_equal, d as dispatch_dev, g as globals, M as Dialog, N as binding_callbacks, O as bind, v as validate_slots, o as onMount, a1 as tick, a7 as onDestroy, A as AH, y as l, af as tag_player, I as beforeUpdate, a8 as afterUpdate, R as Button, T as Checkbox, P as Snackbar, ag as Textfield, w as writable, ab as Loader, e as element, f as space, c as create_component, j as attr_dev, k as add_location, l as set_style, n as insert_dev, p as append_dev, m as mount_component, Y as add_flush_callback, t as transition_in, a as transition_out, x as detach_dev, b as destroy_component, h as text, B as noop } from './main-23c426e8.js';
...
/* clsSMLabSimulation\Labsimulation.svelte generated by Svelte v3.40.2 */

const { console: console_1 } = globals;
const file = "clsSMLabSimulation\\Labsimulation.svelte";

// (354:0) <Dialog        bind:visible={state.remediationToggle}   width={450}   style="background: #fff; border-radius: 5px;"   class="remove_right_margin"  >
function create_default_slot_1(ctx) {
	...
	const block = {
		c: function create() {
			...
			t4 = space();
			t5 = text(t5_value);
			add_location(div0, file, 361, 3, 14924);
			attr_dev(div1, "class", "d-flex justify-content-between");
			add_location(div1, file, 360, 2, 14875);
			attr_dev(h40, "class", "mt-1 font21 mb-4");
			add_location(h40, file, 359, 1, 14842);
			add_location(br, file, 367, 27, 15116);
			add_location(h41, file, 367, 3, 15092);
			attr_dev(center, "class", "mt-xl");
			add_location(center, file, 365, 2, 15040);
			attr_dev(div2, "id", "remediationModel");
			set_style(div2, "overflow-y", "auto");
			set_style(div2, "padding-right", "20px");
			add_location(div2, file, 364, 1, 14962);
		},
	...
	dispatch_dev("SvelteRegisterBlock", {
		...
		type: "slot",
		source: "(354:0) <Dialog        bind:visible={state.remediationToggle}   width={450}   style=\\\"background: #fff; border-radius: 5px;\\\"   class=\\\"remove_right_margin\\\"  >",
		ctx
	});
...
// (372:2) <Button     unelevated={true}              outlined={true}     color="primary"     on:Click={()=>{ state.remediationToggle = false }}    >
function create_default_slot(ctx) {
	...
	dispatch_dev("SvelteRegisterBlock", {
		...
		type: "slot",
		source: "(372:2) <Button     unelevated={true}              outlined={true}     color=\\\"primary\\\"     on:Click={()=>{ state.remediationToggle = false }}    >",
		ctx
	});
...
function create_footer_slot(ctx) {
	...
	const block = {
		c: function create() {
			...
			add_location(div, file, 370, 1, 15167);
		},
		m: function mount(target, anchor) {
...
function instance($$self, $$props, $$invalidate) {
	...
	const unsubscribe = hdd.subscribe(item => {
		$$invalidate(0, state = item);
	});

	// call before the rendering
	onMount(async () => {
		$$invalidate(0, state.xml = editorState.xml, state);
		...
		$$invalidate(1, editorState = { ...editorState, activator: true });
		await tick();
		didMount();
		callOnStateChange();
	});
	...
	const showAns = () => {
		if (state.testMode == '1') {
			$$invalidate(1, editorState = {
				...editorState,
				message: AH.select('#labAnswer').value == '1'
				? 'Correct'
				: 'Incorrect'
			});

			$$invalidate(1, editorState = { ...editorState, snackback: true });
		} else {
			$$invalidate(1, editorState = { ...editorState, snackback: false });
		}
	};
			...
			function () {
				if (state.module == '17') {
					AH.bind('#authoringFrame', 'load', () => {
						...
						$$invalidate(1, editorState = { ...editorState, activator: false });
					});
				}
			},
			200
	...
	const callOnStateChange = async () => {
		...
		if (editorState.toggleMode != state.toggleMode) {
			if (editorState.toggleMode == true) {
				//Test Mode
				$$invalidate(0, state.testMode = 1, state);
				...
				$$invalidate(0, state.testMode = 0, state);
			...
			$$invalidate(0, state.toggleMode = editorState.toggleMode, state);
		}
	};
		...
		try {
			if (state.module == '17') {
				...
				if (tab_data) {
					...
					$$invalidate(0, state.webAutogradeNotCalled = false, state);
	...
	const writable_props = ['editorState', 'ucEditor'];

	Object.keys($$props).forEach(key => {
		if (!~writable_props.indexOf(key) && key.slice(0, 2) !== '$$' && key !== 'slot') console_1.warn(`<Labsimulation> was created with unknown prop '${key}'`);
	});
	...
	$$self.$$set = $$props => {
		...
		if ('ucEditor' in $$props) $$invalidate(2, ucEditor = $$props.ucEditor);
	};
	...
	return [state, editorState, ucEditor, Click_handler, dialog_visible_binding];
}

class Labsimulation extends SvelteComponentDev {
	constructor(options) {
		super(options);
		init(this, options, instance, create_fragment, safe_not_equal, { editorState: 1, ucEditor: 2 });
		...
		if (/*ucEditor*/ ctx[2] === undefined && !('ucEditor' in props)) {
...
//# sourceMappingURL=Labsimulation-ad6627c3.js.map
