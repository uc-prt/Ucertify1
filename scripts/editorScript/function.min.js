var $jscomp=$jscomp||{};$jscomp.scope={};$jscomp.createTemplateTagFirstArg=function(a){return a.raw=a};$jscomp.createTemplateTagFirstArgWithRaw=function(a,b){a.raw=b;return a};function addScript(a){document.body.append('<script class="reactScript">'+a+"\x3c/script>")}function showUcExpStep(a){var b=AI.prevElm(a,".uc_step_explanation");window.curr=b;AI.select(AI.find(b,".uc_step","hidden")[0],"show");0===AI.find(b,".uc_step","hidden").length&&AI.select(a,"hide")}
...
function replaceUnwantedTags(a,b){try{a=a.replace(/&#160;/g," ").replace(/   /g,"&#160; &#160;").replace(/  /g,"&#160;&#160;").replace(/> /g,">&#160;"),a=a.replace(/\$/g,"&#36;"),a=a.replace(/<br><table/g,"<table"),a=a.replace(/<li><br>/g,"<li>"),a=a.replace(/<br><\/li>/g,"</li>"),a=a.replace(/<\/li><br>/g,"</li>"),a=a.replace(/<\/ul><br>/g,"</ul>"),a=a.replace(/<br><\/ul>/g,"</ul>"),a=a.replace(/<\/ol><br>/g,"</ol>"),a=a.replace(/<br><\/ol>/g,"</ol>"),a=a.replace(/<br><li>/g,"<li>"),a=a.replace(/<grammarly-btn([\s\S]*?)<\/grammarly-btn>/g,
""),a=a.replace(/<grammarly([\s\S]*?)<\/grammarly([\s\S]*?)>/g,""),a=a.replace(/<main.*?data-remove="true"[\s\S]*?<\/main>/g,""),a=a.replace(/class=("|')ebook_item_text.*?("|')/g,"class='ebook_item_text'"),a=a.replace(/('|")ebook_item_text('|").*?>/g,"'ebook_item_text'>"),a=a.replace(/('|")ebook_item_text inline('|").*?>/g,"'ebook_item_text inline'>"),a=a.replace(/('|")inline ebook_item_text('|").*?>/g,"'ebook_item_text inline'>"),a=a.replace(/mce-item-table/g,""),a=a.replace(/<input type="hidden".*?>|<input type="hidden".*?\/>/gm,
""),"remediation"==b&&(a=a.replace(/inline_new/,"inline"),a=a.replace(/sec_button inline|sec_button/,"sec_button inline").replace(/ebook_item_text inline|ebook_item_text/,"ebook_item_text inline")),a.replace(/&nbsp;/g," ").replace(/  /g," &nbsp;").replace(/> </g,">&nbsp;<")}catch(c){console.log("It seems incorrect active area / cursor position")}return a}
...
function showHints(a){a=a.closest(".uc_answer_hint");AI.select(AI.find(a,"li","hidden")[0],"show");var b=AI.find(a,"li","hidden").length;0===b&&(b="No",AI.find(a,".hint_show").setAttribute("disabled",!0));AI.find(a,".li_count").innerHTML=b}function requireScript(a,b){var c=document.createElement("script");c.src=a;c.setAttribute("class","reactScript");c.async=!0;c.onload=b;document.body.appendChild(c)}
...
function showWeblinkIframeEditor(a){AI.select(".load_data","hide");var b=a.contentDocument||a.contentWindow.document;a.setAttribute(height,"100%");a.setAttribute(width,"100%");a=document.body.clientHeight;b.querySelector(".editor_close").getAttribute("onclick","window.parent.editorClose()");AI.setCss(b.querySelector("#authoringSection"),{maxHeight:a-140,minHeight:a/2});AI.setCss(b.querySelector("#previewSection"),{minHeight:a/2,maxHeight:a-113})}
function editorClose(){AI.event("#editor_close_btn","click");AI.select(".modal-backdrop.in").remove();AI.select("html").style.overflow="auto"}function getURLParameter(a){for(var b=window.location.search.substring(1).split("&"),c=0;c<b.length;c++){var d=b[c].split("=");if(d[0]==a)return d[1]}}var EXAMMAPPING="";
function checkedExamObjective(a){if(""!=EXAMMAPPING&&"null"!=EXAMMAPPING){var b=EXAMMAPPING;AI.selectAll('input[name="exam_obj[]"]').forEach(function(f){return f.setAttribute("checked","")});AI.select('input[name="exam_obj[]"]').parentElement.style.background="#fff";var c=getURLParameter("content_guid"),d;for(d in b)if(a.previousElementSibling&&a.previousElementSibling.value==d||window.frameElement.getAttribute("guid")==d||c==d){var e=c?c:1!=from_myproject?a.previousElementSibling.value:window.frameElement.getAttribute("guid");
...
function loadExamObjective(a){document.querySelectorAll(".popover").forEach(function(b){return b.style.display="none"});AI.select(".parent_ul").classList.add("list-unstyled");0==document.querySelector(".exam_modal_body").querySelectorAll(".parent_ul li").length?(activate(1),AI.ajax({url:baseUrl+"index_data.php?func=get_exam_obj_list&ajax=1&course_code="+editor.course+"&get_mapping=1&from_myproject="+from_myproject}).then(function(b){b=JSON.parse(b);var c=b.obj;EXAMMAPPING=b.mapping;document.querySelectorAll(".parent_ul li").forEach(function(f){f.remove()});
if("null"!=c)for(var d in c){AI.insert(".parent_ul",'<li class="radio parent_li_'+d+' my-2" tag_guid="'+d+'"><label class="border exam_obj_label d-flex p rounded"><input type="checkbox" class="exam_obj_radio h" id="radio_'+d+'" name="exam_obj[]" value="'+d+'" /><div class="mt-sm"></div><span>'+c[d].n+'</span></label><ul class="child_ul list-unstyled"></ul></li>',"beforeend");for(var e in c[d].c)AI.insert(AI.select(".parent_li_"+d).querySelector(".child_ul"),'<li class="radio child_li pl-4 my-2" tag_guid="'+
e+'" ><label class="border exam_obj_label d-flex p rounded"><input type="checkbox" class="exam_obj_radio h" name="exam_obj[]" id="radio_'+e+'" value="'+e+'" /><div class="mt-sm"></div><span>'+c[d].c[e].n+"</span></label></li>","beforeend")}checkedExamObjective(a);activate(0)})):checkedExamObjective(a)}
function searchText(a){document.querySelectorAll(".exam_modal_body ul > li").forEach(function(b){var c=b.textContent.toUpperCase(),d=a.value.toUpperCase();c.indexOf(d)?b.style.display="block":b.style.display="none"})}
...
function selectObjRadio(a){""==a.getAttribute("checked")?(a.parentElement&&(a.parentElement.style.background="#fff"),AI.selectAll("#assign_exam_obj","removeAttr","disabled"),a.removeAttribute("checked")):(AI.select("#assign_exam_obj").disabled=!0,a.parentElement&&(a.parentElement.style.background="#e3e3eb"))}
